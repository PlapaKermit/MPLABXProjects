...
void aff_lcd_clav(unsigned char c) {
    ...
    switch (c) {
        ...
        case 12 : // valeur touche <
            LCD_FUNC = SHIFT_CURSOR_LEFT; // d�calage de 1 caract�re
            __delay_ms(2);
            acq();
            break;

        case 13 : // valeur touche >
            LCD_FUNC = SHIFT_CURSOR_RIGHT; // d�calage de 1 caract�re
            __delay_ms(2);
            acq();
            break;

        case 14 : // valeur touche AC
            LCD_FUNC = DISPLAY_CLEAR; // remise � z�ro
            __delay_ms(2);
            acq();
            LCD_FUNC = RETURN_HOME; //mettre le curseur au d�but
            __delay_ms(2);
            acq();
            break;
